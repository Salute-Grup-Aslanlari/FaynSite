...
const Branches = ({ images = [] }) => {
  ...
  return (
    <div {...swipeHandlers} className="flex flex-col items-center justify-center gap-6 sm:gap-12 mb-96 py-20 px-4 min-h-[350px] sm:min-h-[800px] md:min-h-[600px] bg-[#E4E0D8]">
      <div className="flex flex-col items-center top-40 mt-4 md:mb-2 sm:mt-16">
        <h2 className="text-black text-3xl sm:text-5xl md:text-6xl lg:text-7xl mb-32" style={{ fontFamily: 'Salina-Book, sans-serif', fontStyle: 'italic' }}>{t('branches.title')}</h2>
      </div>
      ...
      {centerImage && (
        <div className="text-center mt-2 sm:mt-4 md:mt-6">
          ...
          <h3 className="text-black text-xl sm:text-3xl mt-2 sm:mt-4" style={{ fontFamily: 'Salina-Book, sans-serif' }}>{centerImage.title}</h3>
        </div>
      )}
    </div>
  );
